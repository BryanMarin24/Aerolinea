...
public class UsuarioDAO extends HibernateUtil implements IBaseDAO<Usuario, String> {
    ...
    public Usuario consultar(String correo) {
        ...
        try {
            ...
            Query query = getSesion().createQuery("from Usuario p where p.correo = :correo").setParameter("correo", correo);
            query.setMaxResults(1);
            usuario = (Usuario) query.uniqueResult();

            //usuario = (Usuario) getSesion().createQuery("from Usuario p where p.correo = correo and p.contraseña = password").uniqueResult();
        } finally {
            getSesion().close();
        }
        return usuario;
